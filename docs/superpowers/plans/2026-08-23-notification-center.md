...
Publish performs exact `(scope, sourceEventId)` idempotency before any housekeeping. If expiry, partition capacity, orphan cleanup, or source-index capacity requires structural work, commit that clone independently as one `Reset`; only then clone the committed snapshot for the Created/Updated publish commit. Reserve capacity before insertion, choose only records that actually release the constrained index, evict the oldest complete record together with all of its source-index entries, and never emit Created or Toast for an item absent from the committed snapshot. Protect an existing semantic merge target whenever another indexed record can release capacity. If the target's own retained history alone prevents reservation, return stable `NOTIFICATION_SOURCE_INDEX_CAPACITY_EXCEEDED` with no cleanup/save/revision/event/observer mutation, preserving all old source IDs across restart. If the second save fails, the committed Reset remains authoritative while the publish mutation and speculative observer state roll back.
...
Cover 90 days, 1000 per partition including Global, oldest-first deterministic pruning, source-index caps of 10,000 per scope and 50,000 total, a 16 MiB serialized-file cap, a single revision/Reset per structural cleanup, query-time expiry filtering, 24-hour due checks, startup orphan account cleanup (including empty orphan partitions), and reconstruction of active incidents from stored notification history. Test that legacy backfill itself cannot leave an over-cap snapshot, serialized-cap repair uses logarithmic probes and one batch removal, cap eviction only chooses records that release the constrained index, semantic targets are protected when alternatives exist, and a sole target at capacity rejects a new source without forgetting any old source across restart. Oversized future main/temp and unknown-version candidates must remain byte-for-byte intact and block saves. Same-millisecond incident reconstruction uses persisted creating-source index order; legal legacy files with missing or partial order first canonicalize by recovered incident IDs and deterministically leave the latest unmatched unavailable incident active after save/restart.
