...
- Order snapshots only seed observer state. Only command-confirmed terminal results or Realtime `non-final -> final` transitions may create terminal-order notifications. Terminal observer state is absorbing: late non-terminal observations and conflicting terminal states are diagnostic no-ops; a Snapshot-seeded terminal may be notified once only by a later matching Command terminal.
...
**Task 3 schema ruling:** durable absolute idempotency after semantic merges requires retaining every observed `(scope, sourceEventId)`. Task 3 therefore adds the minimal internal `sourceEventIndex` extension above, backfills legacy records on service load, and removes index entries with their target record during delete/clear/prune/account cleanup. `NotificationRecord.sourceEventId` remains the stable creating source; history is not privately encoded into that frontend-facing scalar. This approved Task 3 boundary also centralizes validation for record/input source IDs and dedupe keys in the notification model, including secret-shape checks for every colon-delimited component and delimiter rejection inside policy-level incident/submission components, validates the index in the store, caps the index at 10,000 entries per scope and 50,000 entries total, and caps serialized notification files at 16 MiB. Legacy backfill and partial incident-order canonicalization obey the same caps before the service becomes authoritative; any actual normalization difference is immediately persisted as one copy-on-write revision and `Reset`, even when no record is removed, and failure prevents service construction. Oversized candidates are inspected through one open file handle with a bounded first-root-field schema sniff: the first protected future/unknown candidate remains in place and blocks overwrite when no higher valid v1 exists, while an already valid main remains authoritative over stale lower-priority tmp/bak artifacts.
...
Publish validates the complete input and performs exact `(scope, sourceEventId)` idempotency before any housekeeping. Construct the pending record/UUID/Toast exactly once, then reserve capacity on clones using the exact Created/Updated record, new source-index entry, and final revision including decimal digit growth. If expiry, partition capacity, orphan cleanup, source-index capacity, or the 16 MiB serialized cap requires structural work, commit that clone independently as one `Reset`; only then clone the committed snapshot for the Created/Updated publish commit. Reserve capacity before insertion, choose only records that actually release the constrained resource, evict the oldest complete record together with all of its source-index entries, and never emit Created or Toast for an item absent from the committed snapshot. Protect an existing semantic merge target and the pending Created item. If the target's retained source history alone prevents index reservation, return stable `NOTIFICATION_SOURCE_INDEX_CAPACITY_EXCEEDED`; if no whole-record candidate can satisfy serialized capacity, return stable `NOTIFICATION_FILE_CAPACITY_EXCEEDED`. Both capacity errors have no cleanup/save/revision/event/Toast/observer mutation and are not mapped to `NOTIFICATION_STORAGE_UNAVAILABLE`. If the second save fails, the committed Reset remains authoritative while the publish mutation and speculative observer state roll back; rebuild active incidents after Reset so runtime matches persisted history and restart behavior.
...
Cover 90 days, 1000 per partition including Global, oldest-first deterministic pruning, source-index caps of 10,000 per scope and 50,000 total, a 16 MiB serialized-file cap, a single revision/Reset per structural cleanup, query-time expiry filtering, 24-hour due checks, startup orphan account cleanup (including empty orphan partitions), and reconstruction of active incidents from stored notification history. Test that every actual legal backfill/canonicalization difference is persisted during load with revision+Reset and is stable after restart; a failed startup normalization constructs no service and leaves the authoritative file unchanged. Backfill and prune cannot leave an over-cap snapshot, final-revision digit growth is reserved, serialized-cap repair uses logarithmic probes and one batch removal, cap eviction only chooses records that release the constrained index, semantic targets are protected even when older than alternatives, and a sole target at capacity rejects a new source without forgetting any old source across restart. Near-limit Created and Updated trials must produce Reset then publish with consecutive revisions, have separate phase-one/phase-two COW failure coverage, and prove real-store restart idempotency. File candidate tests use one handle for metadata/read and deterministically cover growth to limit+1; future/unknown candidates remain byte-for-byte intact and block saves unless a valid higher-priority main is already authoritative. Oversized duplicate schema keys obey the first root `schemaVersion`, so leading current/old plus later future is corruption. Same-millisecond incident reconstruction uses persisted creating-source index order; legal legacy files with missing or partial order first canonicalize by recovered incident IDs and deterministically leave the latest unmatched unavailable incident active after save/restart. Terminal order tests cover Snapshot final followed by late Realtime/Snapshot non-final, conflicting Command/Realtime terminal, and one matching Command terminal notification.
