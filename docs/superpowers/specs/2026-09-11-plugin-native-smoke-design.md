...
- Use a separate Tauri configuration in `src-tauri/plugin-smoke/tauri.conf.json` with a distinct application identifier, no automatic windows, and explicitly selected capabilities. The installed macro reads the canonical configuration filename in its supplied path's parent; a differently named file beside the production config is not isolation. Reject inherited `TAURI_CONFIG` overrides in smoke-feature builds rather than merge or alter caller environment. Reuse only `plugin-runtime`, plus one local-main-only smoke completion permission. Do not load default opener/window capabilities implicitly. A checked-in autogenerated completion permission definition is allowed if required by build tooling, but the normal runtime must have neither a completion handler nor any resolved completion grant.
...
- Before profile creation, reject case-insensitive `WEBVIEW2_`/`COREWEBVIEW2_` environment key presence and any existing WebView2 policy root under HKCU/HKLM in either registry view; only missing-key/path results permit launch. Use read-only key-presence probes, no value inspection/logging, no registry/environment edits, and fail on probe errors. This is deliberately conservative: even benign managed-policy roots are unsupported by the developer host. Explicit data-directory arguments alone can be overridden by these mechanisms; see [Microsoft's WebView2 environment reference](https://learn.microsoft.com/en-us/microsoft-edge/webview2/reference/win32/webview2-idl?view=webview2-1.0.4129.50).
