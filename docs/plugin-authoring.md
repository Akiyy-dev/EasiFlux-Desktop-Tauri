...
目前没有同 ID 覆盖更新、签名认证、在线下载和自动更新。需更换受管副本时，先在应用内停用并移除旧副本，再导入新清单；不要手工修改 receipt 或管理索引。受管副本和原始示例文件不同，编辑原文件不会自动更新已导入内容。

所有清单语义（包括导航目标）都绑定启用指纹。只改变 JSON 排版或字段顺序不改变身份；改变语义需要重新确认启用，不能继承旧内容授权。

## 后续生态边界

本阶段交付本地命令、可导入示例和作者契约。后续分别设计受管版本更新/回滚、签名市场与发布链路、更多按最小权限开放的宿主动作；脚本或 WASM 沙箱属于独立阶段。当前没有交易权限或凭据访问接口。
