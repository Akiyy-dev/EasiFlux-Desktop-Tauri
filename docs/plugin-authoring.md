...
本地插件以宿主严格解释的 JSON 清单为入口。v1–v3 只能声明元数据、只读说明和固定页面快捷入口；v4 还可携带一个按固定数值 ABI 运行的受限 WebAssembly 模块。它不是 JavaScript/npm、WASI 或原生动态库机制，也不能访问业务数据。这是一份本地创作指南，不是在线发布市场或通用 SDK。
...
不要把 v2 改成可以导航或计算的清单：导航必须显式使用 v3 或 v4，计算必须显式使用 v4。旧版本宿主拒绝未知版本是兼容性保护，不应通过修改宿主校验绕过。
...
2. 预览中检查动作和目标；对 v4 还要核对代码变更、模块字节数、运行时、ABI 和参数范围。文件超过 16 KiB、未知/缺失/重复字段、非法枚举、错误参数形状均会拒绝。
3. 确认导入后内容仍默认停用。明确启用，在卡片或命令工作台点击验证；计算还必须在表单中再次点击 Run。导入、预览、启用、搜索和渲染不会自动执行来宾。
4. 停用，确认命令不可执行。重新扫描或操作结果不明确时，先确认目录状态再重试，不重放旧操作。

v2/v3/v4 必须有 1–16 个命令，`requestedCapabilities` 必须是 `[]`。其他命令/能力名称不会自动扩展宿主权限。完整尺寸限制与存储规则见[本地清单文档](plugin-local-manifests.md)。
...
所有清单语义（包括导航目标、计算模块字节和参数元数据）都绑定启用指纹。只改变 JSON 排版或字段顺序不改变身份；改变语义需要重新确认启用，不能继承旧内容授权。

## 后续生态边界

已实现本地命令、可导入示例、作者契约、更新前比较和一个封闭的 Wasm 数值 ABI。真正的受管更新/回滚需要不可变版本槽位、权威激活记录和中断恢复，不能由“移除再导入”替代。后续还包括签名市场与发布链路、更多按最小权限开放的宿主动作和其他经过单独设计的运行时。当前计算 ABI 不提供交易、行情、账户、文件、网络或凭据访问接口，也不兼容任意第三方插件。
