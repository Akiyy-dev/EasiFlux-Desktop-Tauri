...
const automatic = new URLSearchParams(window.location.search).get('self-test') === '1'
onMounted(() => {
...
<template>
  <main class="plugin-smoke-host">
    <aside role="note">
      ...
      <p v-if="automatic">Automatic fixture test; native picker and whole app are not tested.</p>
      <p v-else>Manual lane: import, enable/disable, and remove only Plugin Smoke Fixture. Closing does not record an automatic pass.</p>
    </aside>
    <nav v-if="!automatic" aria-label="Plugin section">
      <button @click="section = 'installed'">Installed</button>
      <button @click="section = 'market'">Market</button>
      <button @click="section = 'manage'">Manage</button>
    </nav>
