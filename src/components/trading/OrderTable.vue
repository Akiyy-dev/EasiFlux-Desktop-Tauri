...
import { NButton, NDataTable, NTabPane, NTabs } from 'naive-ui'
...
async function refresh(): Promise<void> {
  ...
  try {
    if (activeScope.value === 'open') {
      await orderStore.refreshOrders()
    } else {
      await orderStore.refreshOrderHistory()
    }
  } catch (e) {
    logStore.setError(e instanceof Error ? e.message : String(e))
  }
...
<template>
  <div class="order-table">
    <div class="toolbar">
      <NTabs v-model:value="activeScope" type="segment" size="small" class="scope-tabs">
        <NTabPane name="open" tab="当前挂单" />
        <NTabPane name="history" tab="历史订单" />
      </NTabs>
