use crate::error::{map_keyring_error, AppError, AppResult};
...
impl CredentialStore {
    pub fn save(account_id: &str, credential: &ApiCredential) -> AppResult<()> {
        ...
        let json =
            serde_json::to_string(credential).map_err(|e| AppError::Config(e.to_string()))?;
        let entry = keyring::Entry::new(KEYRING_SERVICE, account_id).map_err(map_keyring_error)?;
