...
fn permits_absent_configuration_for_non_release_profiles() {
    for profile in ["debug", "test"] {
        assert_eq!(
            validate_news_build_config(profile, None, None).unwrap(),
            None,
...
fn rejects_partial_configuration() {
    let cases = [
        ("debug", Some("https://news.example.test"), None),
        ("test", None, Some("source-20260730")),
        ("release", Some("https://news.example.test"), None),
    ];

    for (profile, base_url, source_epoch) in cases {
        assert!(
            validate_news_build_config(profile, base_url, source_epoch).is_err(),
...
fn accepts_http_only_for_loopback_hosts_outside_release() {
    ...
    for (base_url, normalized_url) in cases {
        let config = validate_news_build_config("debug", Some(base_url), Some("epoch_1"))
...
fn rejects_non_loopback_http_and_all_release_http() {
    for (profile, base_url) in [
        ...
        assert!(
            validate_news_build_config(profile, Some(base_url), Some("epoch_1")).is_err(),
...
fn release_requires_complete_https_configuration() {
    assert!(validate_news_build_config("release", None, None).is_err());
...
fn rejects_url_credentials_query_and_fragment() {
    for base_url in [
        ...
        assert!(
            validate_news_build_config("debug", Some(base_url), Some("epoch_1")).is_err(),
...
fn validates_source_epoch() {
    for epoch in ["A", "source.2026_07-30", &"a".repeat(64)] {
        assert!(
            validate_news_build_config("debug", Some("https://news.example.test"), Some(epoch))
                .is_ok(),
            "should accept {epoch:?}"
        );
    }
...
fn error_display_is_category_only_and_redacts_rejected_input() {
    ...
    let error = validate_news_build_config("release", Some(rejected_url), Some("epoch_1"))
